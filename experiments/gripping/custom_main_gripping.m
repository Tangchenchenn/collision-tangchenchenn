...
%% actuation
kappa_bar_total = [0.0628525320866699*3.*ones(n_bend_twist,1) , zeros(n_bend_twist,1)]; % 3quartercircle
kappa_bar_total(1:10,:) = zeros(10,2);
...
for timeStep = 1:Nsteps
    ...
    %% actuation
    current_kappa_bar = kappa_bar_total.*(timeStep/Nsteps);
    ...
    if mod(timeStep, sim_params.plotStep) == 0
        sphere_gd_plot_MultiRod(softRobot, ctime, sim_params, environment, imc);
